/**
 * [FIRST VERSION]
 * Parallel Trial Division Algorithm
 * This program factors a given number using multiple threads.
 */
...
void findPrimesInRange(unsigned long long start, unsigned long long end, unsigned long long num, vector<factor_exponent>& primes) {
    ...
    for (unsigned long long i = start; i <= end; ++i) {
        ...
        if (isPrime(i) && num % i == 0) {
            ...
            // Add prime factor and its exponent to the vector
...
vector<factor_exponent> parallelTrialDivision(unsigned long long num, int numThreads) {
    ...
    for (int i = 0; i < (numThreads - 1); ++i) {
        // Start a thread with a specific range
        threads.emplace_back(findPrimesInRange, start, end, num, ref(primes));
        // Update the start for the next thread
        start = end + 1;
        // Update the end for the next thread
        end = start + range - 1;
    }
    // Last thread processes up to the square root of num
    end = sqrt_num;
    ...
    if (primes.empty()) {
        ...
        // If product is different from num, add the missing prime factor
        if (product != num) {
